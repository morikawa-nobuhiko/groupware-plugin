...
import jp.groupsession.v2.struts.AbstractGsForm;
	...
	* @author JTS
	*/
	public class Knt302Form extends AbstractGsForm {

	/** プロジェクトSID */
	private int prjSid__ = 0;
	/** プロジェクトID */
	private int delPrjSid__ = -1;
	/** プロジェクトID */
	...
	throws SQLException {
		...
		int psi = this.getPrjSid();
		if (psi < 0) {
			errors.add("prjSid", new ActionMessage("プロジェクトSIDは必須項目です。"));
		}

		String pi = this.getPrjId();
		if (pi.equals("")) {
			errors.add("prjId", new ActionMessage("プロジェクトIDは必須項目です。"));
		}

		String pn = this.getPrjName();
		pn = pn.replaceAll(" ","");
		pn = pn.replaceAll("　","");
		if (pn == "") {
			errors.add("prjName", new ActionMessage("プロジェクト名称は必須項目です。"));
		}

		Calendar calS = Calendar.getInstance();
		Calendar calE = Calendar.getInstance();
		calS.set(this.getStartYear(), this.getStartMonth()-1, this.getStartDay());
		calE.set(this.getEndYear(), this.getEndMonth()-1, this.getEndDay());
		if (calE.before(calS)) {
	 ...
	 * @return delPrjSid
	 */
	public int getDelPrjSid() {
		return delPrjSid__;
	}

	/**
	 * <p>delPrjSid をセットします。
	 * @param delPrjSid delPrjSid
	 */
	public void setDelPrjSid(int delPrjSid) {
		delPrjSid__ = delPrjSid;
	}
